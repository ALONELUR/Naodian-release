classdef  newWheelSSVEP < handle
    ...
    methods (Access = public)
        % 此函数是程序连续进行分析的程序
        function mode1(app)
            app.EEG_Driver.OfflineInit(app.Gazetime);
            
             while ~get(app.scom, 'BytesAvailable')
                app.EEG_Driver.ReadStart();
                app.EEG_Analysiser.LoadData(app.EEG_Driver.DataSequenceOutput());
                app.EEG_Analysiser.Filter();
                app.EEG_Analysiser.CCA();
                app.TotalResult = app.EEG_Analysiser.CCAResult();
                app.judgeANDsend();
            end
           app.pause();
        end
...
%                     app.threshold = ones(1,8);
    ...
    methods (Access = private)
        ...
        function init(app)
            ...
            SampleRate = app.Samplefreq;
            SampleTime = app.Gazetime;
            TargetFre = app.Tragetfreq;
            decivetype = 8;
            harms = 1;
            app.EEG_Analysiser = EEG_Analysis(SampleRate, SampleTime, decivetype,TargetFre, harms);
            
            sCOM = app.Num_Data_com_n;
            Rate = SampleRate;
            Gain = 8;
            app.EEG_Driver = EEG_Drive(decivetype,sCOM,Rate,Gain);
