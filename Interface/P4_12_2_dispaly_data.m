...
for i = 1:length(frequ)
    for j = 1:216000 
        rea_col(i,j) = 255;%0.5*255*(1+sin((2*pi*frequ(i))*(j/fre)+0.5*(i-1)*pi));
    end 
end
...
WaitSecs(0.01);
while ~KeyCode(escapeKey)&&~sum(buttons)&&indexflip~=36000
    [KeyIsDown, secs, KeyCode] = KbCheck;
    [x,y,buttons,focus,valuators,vailnfo]=GetMouse(window);
...
%         Screen('DrawText',window,double('仪器科学与光电工程学学院'),1.225*x_center+0.2*y_center,1.9*y_center-text_size/3,255);
    ...
    strRec=[85,save_data(indexflip)+48,10];
    if 1
        Color=ones(8,3);
        Color(8,:)=[1,0,0];
        if strRec(2)>=49&&strRec(2)<=56
            ...
            Timer=0;
            if strRec(2)==53&&save_data(indexflip-1)~=5
                if Speed_flag~=3
            ...
            if strRec(2)==54&&save_data(indexflip-1)~=6
                if Speed_flag~=1
                    Speed_flag=Speed_flag-1;
                    send_message=6;
                else
